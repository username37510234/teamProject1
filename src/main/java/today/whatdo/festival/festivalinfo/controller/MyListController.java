...
public class MyListController {
	...
	@GetMapping("/my-lists")
	@ResponseBody
	public List<MyListVO> getDibInformationList(@ModelAttribute MyListVO myList, Model model, HttpSession session){
	...
	@ResponseBody
	public int insertDibInformation(@RequestBody MyListVO myList, HttpSession session) {
		UserInfoVO userInfo = (UserInfoVO)session.getAttribute("userInfo");
		if(userInfo == null) {
			throw new RuntimeException("로그인이 필요합니다.");
		}
	...
	@DeleteMapping("/my-lists/{mlNum}")
	@ResponseBody
	public int deleteBoardInfo(@PathVariable int mlNum, HttpSession session) {
		UserInfoVO userInfo = (UserInfoVO)session.getAttribute("userInfo");
		if(userInfo == null) {
			throw new RuntimeException("로그인이 필요합니다.");
		}
		...
		myList.setUiNum(userInfo.getUiNum());
		return myListService.deleteMyList(mlNum);
	}
	
	//로그인 돼있어야 마이리스트 선택 삭제 가능
	@DeleteMapping("/my-lists")
	@ResponseBody
	public int deleteBoardInfos(@RequestBody MyListVO myList, HttpSession session) {
		UserInfoVO userInfo = (UserInfoVO)session.getAttribute("userInfo");
		if(userInfo == null) {
			throw new RuntimeException("로그인이 필요합니다.");
		}
